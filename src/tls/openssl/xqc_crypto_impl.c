 ...
 *
 * OpenSSL 3.0 crypto implementation.
 * Provides AEAD encrypt/decrypt, header-protection mask, and context
 * management using the OpenSSL 3.0 EVP_CIPHER API.
 *
 * Key differences from BabaSSL backend:
 *   - Function names prefixed with xqc_ossl3_ to avoid symbol conflicts.
 *   - Uses EVP_CIPHER_CTX_ctrl with EVP_CTRL_AEAD_* for GCM tag handling,
 *     same pattern as the BabaSSL backend.
 *   - ChaCha20 header-protection uses EVP_CIPHER-based EVP_chacha20() 
 *     instead of the BoringSSL-specific CRYPTO_chacha_20().
 */
...
xqc_int_t
xqc_ossl3_aead_encrypt(const xqc_pkt_protect_aead_t *pp_aead, void *aead_ctx,
...
xqc_int_t
xqc_ossl3_aead_decrypt(const xqc_pkt_protect_aead_t *pp_aead, void *aead_ctx,
    uint8_t *dest, size_t destcap, size_t *destlen,
    ...
    const uint8_t *ad, size_t adlen)
{
    ...
    if (taglen > (ssize_t)ciphertextlen
        || ciphertextlen > destcap + xqc_aead_overhead(pp_aead, destcap))
    {
        return -XQC_TLS_NOBUF;
    }
...
xqc_int_t
xqc_ossl3_hp_mask(const xqc_hdr_protect_cipher_t *hp_cipher, void *hp_ctx,
    uint8_t *dest, size_t destcap, size_t *destlen,
    const uint8_t *plaintext, size_t plaintextlen,
    const uint8_t *key, size_t keylen,
