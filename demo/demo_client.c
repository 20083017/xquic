    ...
    xqc_demo_cli_client_args_t *args)
{
    ...
    {
        size_t co_len = strlen(args->quic_cfg.co_str);
        if (co_len >= XQC_CO_STR_MAX_LEN) {
            co_len = XQC_CO_STR_MAX_LEN - 1;
        }
        memcpy(settings->conn_option_str, args->quic_cfg.co_str, co_len);
        settings->conn_option_str[co_len] = '\0';
    }
}
